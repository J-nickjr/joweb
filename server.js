...
// ✅ 替換 mysql2 改用 pg 套件連接 Supabase PostgreSQL
const express = require('express');
...
db.connect()
  .then(() => console.log('✅ 成功連線到 Supabase PostgreSQL'))
  .catch((err) => console.error('❌ PostgreSQL 連線錯誤:', err));
...
app.post('/api/login', (req, res) => {
  const { email, password } = req.body;
  db.query('SELECT * FROM members WHERE email = $1', [email])
    .then(async (result) => {
      if (result.rows.length === 0) return res.status(401).json({ message: '查無此帳號，請註冊' });
    ...
    .catch(() => res.status(500).json({ message: '伺服器錯誤' }));
});
...
app.post('/api/member-info', (req, res) => {
  ...
  const sql = 'SELECT email, phone, name, birth, gender, is_subscribed FROM members WHERE email = $1';
  db.query(sql, [email])
    .then((result) => {
      if (result.rows.length === 0) return res.status(404).json({ message: '查無會員資料' });
...
app.post('/api/register', async (req, res) => {
  ...
  if (!record) return res.status(400).json({ message: '❌ 無驗證記錄' });
  if (String(record.code) !== String(code)) return res.status(400).json({ message: '❌ 驗證碼錯誤' });
  if (record.expiresAt < now) return res.status(400).json({ message: '❌ 驗證碼已過期' });

  const hashedPassword = await bcrypt.hash(password, 10);
  const sql = 'INSERT INTO members (email, phone, password, is_subscribed) VALUES ($1, $2, $3, $4)';
  db.query(sql, [email, phone, hashedPassword, is_subscribed || false])
    .then(() => {
      verificationCodes.delete(phone);
      res.json({ message: '✅ 註冊成功' });
    })
    .catch((err) => {
      ...
      if (err.code === '23505') return res.status(400).json({ message: '❌ 此 Email 已被註冊' });
      res.status(500).json({ message: '❌ 註冊失敗（資料庫錯誤）' });
    });
});
...
app.post('/api/update-member', (req, res) => {
  ...
  const sql = `
    UPDATE members
    SET name = $1, gender = $2, birth = $3, phone = $4, is_subscribed = $5
    WHERE email = $6
  `;

  db.query(sql, [name, gender, birth, phone, is_subscribed ? true : false, email])
    .then((result) => {
      if (result.rowCount === 0) return res.status(404).json({ message: '找不到該會員帳號' });
      res.json({ message: '✅ 資料已成功更新' });
    })
    .catch((err) => {
      console.error('❌ 更新會員資料失敗：', err);
